...
if (!empty($_POST['fullname'])) {
    ...
    if (!empty($fullname)) {
        ...
        if ($id_user == '') {
            // Thêm khách hàng
            $sql = 'INSERT INTO user(fullname, tendangnhap, email, diachi, matkhau, dienthoai, role) 
        VALUES ("' . $fullname . '", "' . $tendangnhap . '", "' . $email . '", "' . $diachi . '", "' . $matkhau . '", "' . $dienthoai . '", "user")';
        } 
        else {
            // Sửa khách hàng
...
<div class="d-flex flex-column flex-lg-row h-lg-full bg-surface-secondary">
    ...
    if (isset($_GET['id_user'])) {
        ...
        echo '<h5 class="mb-0">Edit Customer</h5>';
    } else {
        // Nếu không, hiển thị "Add Customer"
        echo '<h5 class="mb-0">Add Customer</h5>';
    }
    ?>
</div>
    ...
    <script type="text/javascript">
        function addUser()
        {
            var option = confirm('Add a new customer success')
            if (!option) {
                return;
            }
        }
